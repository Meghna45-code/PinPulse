...
class PinPulseEngine:
    ...
    def _get_cache_key(self, zip_code, state, festival_active):
        return f"{zip_code}_{state}_{festival_active}"
    ...
    def score_all_products(self, user_context):
        ...
        # State Hysteresis: Check cache
        zip_info = self.zip_data.get(zip_code, {})
        festival_active = zip_info.get("active_festival", None) is not None
        cache_key = self._get_cache_key(zip_code, state, festival_active)
        
        now = time.time()
        if (cache_key in self._cache and 
            now - self._cache_timestamp < CACHE_TTL_SECONDS and
            not session_cart):  # Bypass cache if cart has items
        ...
        weather_condition = zip_info.get("weather_conditions", "hot_humid")
        active_festival = zip_info.get("active_festival", None)
        zip_aov = zip_info.get("aov", 2500)

        # Get allowable materials from weather rules
        allowable_materials = self.weather_rules.get(
            weather_condition, {}
        ).get("allowable_materials", ["cotton", "linen"])
        allowable_materials_vector = self.weather_rules.get(
            weather_condition, {}
        ).get("vector", [])
        ...
        for interaction in interactions:
            ...
            adj["hours_elapsed"] = interaction["hours_elapsed"] + time_offset
        ...
        for product in catalog_with_intent:
            ...
            # === WEATHER HARD VETO ===
            if s_fabric < 0.2:
                continue  # Disqualify this product entirely

            # === PILLAR 3: Festivity ===
            s_festivity = calculate_festivity_score(
            ...
            if session_cart:
                for cart_item_id in session_cart:
                    if cart_item_id in self.cf_lookup:
                        for rec in self.cf_lookup[cart_item_id]["recommendations"]:
    ...
    def simulate_velocity_surge(self, zip_code):
        ...
        Returns hardcoded theme + matched items (faked LLM for demo).
        """
        # Hardcoded themes per ZIP for instant demo
        themes = {
            "800008": {
                "theme": "Midnight Blue Festive Bodycons & Modern Lehengas",
                "matched_ids": ["prod_003", "prod_007", "prod_012", "prod_045", "prod_088"],
            },
            "530001": {
                "theme": "Emerald Green Occasion Wear",
                "matched_ids": ["prod_005", "prod_022", "prod_067", "prod_091", "prod_102"],
            },
            "641001": {
                "theme": "Rose Gold Silk Sarees & Traditional Drapes",
                "matched_ids": ["prod_001", "prod_015", "prod_034", "prod_056", "prod_078"],
            },
        }
        
        default_theme = {
            "theme": "Vibrant Ethnic Fusion Wear",
            "matched_ids": ["prod_001", "prod_003", "prod_005", "prod_007", "prod_012"],
        }
